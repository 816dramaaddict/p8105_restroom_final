...
For restroom data, we factor the character variables (`status`, `open`, `accessibility`, `changing_stations`) into binary and categorical variables. We handle NAs in `status` and `changing_stations` by assigning 0, and `open`, `accessibility` by adding explicity NA level `'Unknown'`
