...
subway_with_restroom = read_csv("Data/cleaned_subway_restroom_data.csv") %>% 
  mutate(
    ada <- factor(ada, levels = c("FALSE", "TRUE")),
    free_crossover <- factor(free_crossover, levels = c("FALSE", "TRUE")),
    # restroom_status <- factor(restroom_status, levels = c("FALSE", "TRUE")),
    # restroom_changing_stations <- factor(restroom_changing_stations, levels = c("FALSE", "TRUE"))
    restroom_changing_stations <- case_when(
      restroom_changing_stations == NA ~ 0
    )
  )
...
clustering_merged_data <- subway_with_restroom %>%
  dplyr::select(entrance_type, staffing, ada, free_crossover, station_latitude, station_longitude, station_name,
                restroom_open, restroom_accessibility, restroom_changing_stations, restroom_status)
